...
- ✅ **Wave 4** — `GET /api/public/verified` paginated/filterable feed (category, grade, min_score, min_confidence, cursor pagination, `spx.verified.v1` schema, edge-cached `s-maxage=300, swr=3600`). `/embed/$subject` iframe widget (badge + last-5 events + confidence chip, SSR-loaded, `noindex`). `/methodology` rewritten as versioned public commitment (current versions panel, two-axis risk×confidence section, full event taxonomy table, blind spots, false-positive policy, appeals + dispute-window policy, freshness SLA, retroactive scoring policy, schema changelog, machine-readable surfaces index). Embed widget + Evidence bundle linked from agent dossier action row.
- ⏭ **Next: Wave 5** — SAS attestations. Will request `SPX_ATTESTATION_KEYPAIR` secret then ship `cron-attest` (mint/refresh attestations on hysteresis-thresholded score/grade/bond changes), `agent_attestations` mirror table, `spx.reputation.v1` payload (carries methodology versions + evidence_root from Wave 1c), `attestation_id` populated on Evidence Bundle.
