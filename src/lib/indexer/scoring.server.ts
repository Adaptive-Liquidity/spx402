...
interface LegacyScoreBreakdown extends ScoreBreakdown {
  depositConsistency: number;
  buybackExecution: number;
  burnConfirmation: number;
  metadata: number;
}

interface AeonScoreBreakdown extends ScoreBreakdown {
  escrowCompletion: number;
  slashableBond: number;
}
...
export function score(inputs: ScoringInputs): ScoreResult {
  if (hasAeonExecutionInputs(inputs)) {
...
function scoreTokenized(inputs: ScoringInputs): ScoreResult {
  ...
  const total = sumLegacyBreakdown(breakdown);
  return {
    total,
    breakdown,
    grade: gradeForTokenized(total, inputs),
    verdict: verdictForTokenized(total, inputs),
    confidence: confidenceForLegacy(inputs),
  };
}
...
function scoreRegistered(inputs: ScoringInputs): ScoreResult {
  ...
  return {
    ...
    confidence: confidenceForLegacy({
      ...inputs,
...
// ─────────────────────────────────────────────────────────────────────
// Branch 5: AEON execution scoring
// ─────────────────────────────────────────────────────────────────────
function scoreAeonExecution(inputs: ScoringInputs): ScoreResult {
  ...
  const breakdown: AeonScoreBreakdown = {
    escrowCompletion:
      totalEscrows > 0 ? clamp(Math.round(successRate * 40), 0, 40) : 0,
    slashableBond: aeonSlashableBond(activeBond, slashedUsd),
    failedTx: clamp(15 - (inputs.failedWindows + escrowsFailed) * 3, 0, 15),
    recency: recencyScore(inputs.lastIndexedSeconds, "short"),
    operator: inputs.operatorVerified ? 5 : 0,
  };
  const total = sumAeonBreakdown(breakdown);

  return {
    total,
    breakdown,
    grade:
      totalEscrows === 0 && activeBond === 0 && inputs.totalDepositsCount === 0
        ? "SPX404"
        : gradeFromTotal(total),
...
// Shared helpers
// ─────────────────────────────────────────────────────────────────────
function hasAeonExecutionInputs(inputs: ScoringInputs): boolean {
  return (
    inputs.totalEscrowsCompleted !== undefined ||
    inputs.totalEscrowsFailed !== undefined ||
    inputs.escrowSuccessRate !== undefined ||
    inputs.activeBondAmount !== undefined ||
    inputs.totalSlashedUsd !== undefined
  );
}
...
function sumLegacyBreakdown(b: LegacyScoreBreakdown): number {
  return (
...
function sumAeonBreakdown(b: AeonScoreBreakdown): number {
  return b.escrowCompletion + b.slashableBond + b.failedTx + b.recency + b.operator;
}

function aeonSlashableBond(activeBond: number, slashedUsd: number): number {
  let points = clamp(Math.round((activeBond / 10000) * 30), 0, 30);
  if (slashedUsd > 0) {
    points = Math.max(0, points - clamp(Math.round((slashedUsd / 1000) * 10), 0, 30));
  }
  return points;
}
...
function confidenceForLegacy(i: ScoringInputs): "high" | "medium" | "low" {
