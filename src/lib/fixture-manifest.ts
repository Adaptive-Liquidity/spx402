...
export const FIXTURE_MANIFEST: FixtureManifestEntry[] = [
  {
    ...
    skipReason:
      "No tracked tokenized agent currently has a non-null deposit_address, so no real deposit tx exists to capture.",
  },
  {
    ...
    signature:
      "4z2jZZwLhPBkjevfNADaPFeKHQtfUAPsX2dNTndTrXbSG2Gdo6ZevqjD6bHAdvKRcNQwBH5VuBYGsdiiWEc44G8L",
    parserVersionIntroduced: "spx-decoder-v1",
    skipReason: null,
  },
  {
    ...
    parserVersionIntroduced: "spx-decoder-v1",
    skipReason:
      "Awaiting verbatim capture; no burn observed for the two live indexed agents in the recent window.",
  },
  {
  ...
  },
  {
    ...
    signature:
      "3tKr9rBF2G4TreVbqzbwpH5UEfSgbEoxfLJMCq4WfraTyHvktUVALT3MgipmtRsQn9ktQ4rj7fAFXQifBBvdKB9d",
    parserVersionIntroduced: "spx-decoder-v1",
    skipReason: null,
  },
  {
  ...
  },
  {
    ...
    parserVersionIntroduced: "spx-decoder-v1",
    skipReason:
      "Zero x402 executors are indexed in production; facilitator-registry detection ships in the Facilitator Registry Detection Patch.",
  },
  {
  ...
  },
  {
    ...
    parserVersionIntroduced: "spx-decoder-v1",
    skipReason:
      "No executor wallets are indexed yet, so no swap tx can be attributed to a tracked wallet.",
  },
  {
    id: "C2_raydium_swap",
    section: "C · Negative controls",
  ...
  },
  {
    ...
    skipReason:
      "No Base facilitator sender address is published-and-verified yet. cdp-base and payai-base are registered address-less and INACTIVE; the lane ships report-only. Capture once an operator publishes a Base sender: bun scripts/capture-fixture.ts --chain base --id E1_facilitator_transfer_with_authorization --signature 0x...",
  },
  {
    ...
    parserVersionIntroduced: "v1.0.0-evm",
    skipReason:
      "Depends on E1: no Base facilitator is active, so no registry-sender Permit2 settlement can be attributed. The Permit2 selector is derived from the canonical ABI at build time and stays UNPINNED until this fixture is captured.",
  },
  {
  ...
  },
  {
    ...
    signature:
      "5C4AUZEZeYPpwpjRsZWpmG3cxBhdiq5NtdHmt8YPUaXUpnwnELfbZHgUc9g9GNghHopmzsr4ZB8Q1kTWUq9YB2JF",
  ...
  },
  {
    ...
    signature:
      "2V6r5myBt1iEYpLbaQbwGLc7iprxiQ4NLdabyRC9zxAmx9pio8BaTutfKd1Y2pPMeCs43P42ZCc7zrPUVM7gtkJ2",
    parserVersionIntroduced: "v0.2.0",
    skipReason: null,
  },
];
...
export function fixtureTotals(): { total: number; captured: number; lastCaptureAt: string | null } {
  ...
  const last = FIXTURE_MANIFEST.map((f) => f.capturedAt)
    .filter(Boolean)
    .sort();
  return {
