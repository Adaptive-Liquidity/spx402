import type { TemplateData } from "./registry";
import {
  Body,
  Button,
  Container,
...
function SpxAlertEmail({
  ...
  // Runtime data arrives unvalidated (callers cast); a null/non-string event
  // must not crash rendering. One normalized value for Preview and Heading.
  const displayEvent = typeof event === "string" ? event : "ESCROW_CREATED";
  return (
    <Html>
      <Head />
      <Preview>{`SPX402 alert — ${displayEvent}`}</Preview>
...
export const template = {
  ...
  subject: (d: TemplateData) =>
    `SPX402 alert — ${typeof d.event === "string" ? d.event : "agent event"}`,
